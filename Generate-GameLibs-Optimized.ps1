...
$ModSDKExtraDLLs = @(
    ...
    "Unity.TextMeshPro.dll", "UnityEngine.ParticleSystemModule.dll", "UnityEngine.TerrainModule.dll",
    ...
    "FMODAssembly.dll", "Cinemachine.dll", "Unity.Timeline.dll",
...
if (Test-Path $propsSource) {
    ...
    Write-Host "   [OK] GameLibs.props" -ForegroundColor Green
}
...
Write-Host "   [PackageSet] ModSDK: $($modSDKFound.Count) DLL files copied" -ForegroundColor Green
...
if ($criticalFound.Count -gt 0) {
    ...
    Write-Host "   [Archive] Critical: $ArchiveCritical ($criticalSizeMB MB)" -ForegroundColor Green
}
...
if ($modSDKFound.Count -gt 0) {
    ...
    Write-Host "   [Archive] ModSDK: $ArchiveModSDK ($modSDKSizeMB MB)" -ForegroundColor Green
}
...
if ($modSDKMissing.Count -gt 0) {
    ...
    foreach ($dll in $modSDKMissing | Select-Object -First 10) {
        Write-Host "     - $dll" -ForegroundColor Gray
    }
    if ($modSDKMissing.Count -gt 10) {
        Write-Host "     ... and $($modSDKMissing.Count - 10) more" -ForegroundColor Gray
    }
}
...
Write-Host "   • Consider committing archives vs full directories" -ForegroundColor White
...
Write-Host ""
Write-Host "Optimized GameLibs ready for CI/CD!" -ForegroundColor Green